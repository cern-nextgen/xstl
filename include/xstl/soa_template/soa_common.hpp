...
namespace cms::soa {
  ...
  struct SoAConstParametersImpl {
    ...
    SOA_HOST_DEVICE SOA_INLINE constexpr SoAConstParametersImpl(ValueType const* addr)
        : addr_(addr) {}

    // constructor from a non-const parameter set
    SOA_HOST_DEVICE SOA_INLINE constexpr SoAConstParametersImpl(
  ...
  struct SoAConstParametersImpl<SoAColumnType::eigen, T> {
    ...
    static constexpr bool checkAlignment(TupleOrPointerType const& tuple,
  ...
  template <SoAColumnType COLUMN_TYPE, typename T>
  SOA_HOST_DEVICE SOA_INLINE constexpr SoAParametersImpl<COLUMN_TYPE, T>
  const_cast_SoAParametersImpl(SoAConstParametersImpl<COLUMN_TYPE, T> const& o) {
    return SoAParametersImpl<COLUMN_TYPE, T>{non_const_ptr(o.addr_)};
  }

  template <typename T>
  SOA_HOST_DEVICE SOA_INLINE constexpr SoAParametersImpl<SoAColumnType::eigen, T>
  ...
  public:
    ...
    SOA_HOST_DEVICE SOA_INLINE SoAValue(size_type i,
                                        SoAParametersImpl<SoAColumnType::eigen, C> params)
        : val_(params.addr_ + i,
               C::RowsAtCompileTime,
               C::ColsAtCompileTime,
  ...
  class SoAValue<SoAColumnType::eigen, C, ALIGNMENT, RESTRICT_QUALIFY> {
    static_assert(!sizeof(C),
                  "Eigen/Core should be pre-included before the SoA headers to enable support for "
                  "Eigen columns.");
  };
#endif
  ...
  public:
    ...
    SOA_HOST_DEVICE SOA_INLINE SoAConstValue(size_type i,
                                             SoAConstParametersImpl<COLUMN_TYPE, T> params)
        : idx_(i), col_(params.addr_) {}
  ...
  struct EigenConstMapMaker {
    using Type =
        Eigen::Map<const C, Eigen::AlignmentType::Unaligned, Eigen::InnerStride<Eigen::Dynamic>>;
    ...
    public:
      ...
      EigenConstMapMaker::Type withStride(byte_size_type stride) {
        return EigenConstMapMaker::Type(data_,
                                        C::RowsAtCompileTime,
                                        C::ColsAtCompileTime,
                                        Eigen::InnerStride<Eigen::Dynamic>(stride));
      }
...
namespace cms::soa {
  ...
  struct SoAColumnAccessorsImpl<T,
                                SoAColumnType::column,
                                SoAAccessType::mutableAccess,
                                alignment,
                                restrictQualify> {
    SOA_HOST_DEVICE SOA_INLINE
    SoAColumnAccessorsImpl(const SoAParametersImpl<SoAColumnType::column, T>& params)
        : params_(params) {}
  ...
  struct SoAColumnAccessorsImpl<T,
    ...
    SOA_HOST_DEVICE SOA_INLINE T const& operator()(size_type index) const {
      return params_.addr_[index];
    }
  ...
  template <typename T, byte_size_type alignment, bool restrictQualify>
  struct SoAColumnAccessorsImpl<T,
                                SoAColumnType::scalar,
                                SoAAccessType::mutableAccess,
                                alignment,
                                restrictQualify> {
    SOA_HOST_DEVICE SOA_INLINE
    SoAColumnAccessorsImpl(const SoAParametersImpl<SoAColumnType::scalar, T>& params)
        : params_(params) {}
    SOA_HOST_DEVICE SOA_INLINE T& operator()() { return *params_.addr_; }
    using NoParamReturnType = T&;
  ...
  struct SoAColumnAccessorsImpl<T,
                                ...
                                restrictQualify> {
    SOA_HOST_DEVICE SOA_INLINE
    SoAColumnAccessorsImpl(const SoAParametersImpl<SoAColumnType::eigen, T>& params)
        : params_(params) {}
    SOA_HOST_DEVICE SOA_INLINE typename T::Scalar* operator()() { return params_.addr_; }
    using NoParamReturnType = typename T::Scalar*;
    using ParamReturnType =
        typename SoAValue<SoAColumnType::eigen, T, alignment, restrictQualify>::MapType;
  ...
  struct SoAColumnAccessorsImpl<T,
    ...
    SoAColumnAccessorsImpl(const SoAConstParametersImpl<SoAColumnType::eigen, T>& params)
        : params_(params) {}
    SOA_HOST_DEVICE SOA_INLINE typename T::Scalar const* operator()() const {
      return params_.addr_;
    }
    using NoParamReturnType = typename T::Scalar const*;
    using ParamReturnType =
        typename SoAValue<SoAColumnType::eigen, T, alignment, restrictQualify>::CMapType;
      ...
      struct AccessType {
        ...
        struct Alignment {
          ...
          struct RestrictQualifier
            ...
            using SoAColumnAccessorsImpl<T, columnType, accessType, alignment, restrictQualify>::
...
template <typename SOA,
          typename SFINAE = typename std::enable_if_t<
              std::is_invocable_v<decltype(&SOA::soaToStreamInternal), SOA&, std::ostream&>>>
...
namespace cms::soa::detail {
  ...
  void printColumn(
    ...
    soa_impl_os << " Scalar " << name << " at offset " << soa_impl_offset << " has size "
                << sizeof(T) << " and padding "
                << ((sizeof(T) - 1) / alignment + 1) * alignment - sizeof(T) << std::endl;
  ...
  void printColumn(
      ...
      cms::soa::byte_size_type alignment) {
    soa_impl_os << " Column " << name << " at offset " << soa_impl_offset << " has size "
                << sizeof(T) * elements << " and padding "
                << cms::soa::alignSize(elements * sizeof(T), alignment) - (elements * sizeof(T))
                << std::endl;
  ...
  void printColumn(std::ostream& soa_impl_os,
    ...
    soa_impl_os << " Eigen value " << name << " at offset " << soa_impl_offset << " has dimension "
                ...
                << cms::soa::alignSize(elements * sizeof(T::Scalar), alignment) -
                       (elements * sizeof(T::Scalar))
                << std::endl;
    soa_impl_offset += cms::soa::alignSize(elements * sizeof(T::Scalar), alignment) *
  ...
  struct AccumulateColumnByteSizes<cms::soa::SoAParametersImpl<cms::soa::SoAColumnType::scalar, T>> {
    cms::soa::byte_size_type operator()(cms::soa::size_type,
  ...
  struct AccumulateColumnByteSizes<cms::soa::SoAParametersImpl<cms::soa::SoAColumnType::column, T>> {
    cms::soa::byte_size_type operator()(cms::soa::size_type elements,
                                        cms::soa::byte_size_type alignment) const {
  ...
  struct AccumulateColumnByteSizes<cms::soa::SoAParametersImpl<cms::soa::SoAColumnType::eigen, T>> {
    cms::soa::byte_size_type operator()(cms::soa::size_type elements,
                                        cms::soa::byte_size_type alignment) const {
      return cms::soa::alignSize(elements * sizeof(typename T::Scalar), alignment) *
             T::RowsAtCompileTime * T::ColsAtCompileTime;
    }
  };
  ...
  auto getSpanToColumn(const cms::soa::SoAParametersImpl<cms::soa::SoAColumnType::eigen, T>& column,
    ...
    return std::span(column.addr_,
                     cms::soa::alignSize(elements * sizeof(typename T::Scalar), alignment) *
                         T::RowsAtCompileTime * T::ColsAtCompileTime / sizeof(typename T::Scalar));
  }

  template <typename T>
  auto getSpanToColumn(
      const cms::soa::SoAConstParametersImpl<cms::soa::SoAColumnType::scalar, T>& column,
      cms::soa::size_type elements,
      cms::soa::byte_size_type alignment) {
    return std::span(column.addr_, 1);
